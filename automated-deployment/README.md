...
5. Modify the ```parameters.json``` file with relevant values:
    ...
    [
        {
            "ParameterKey": "pTargetAccountId", # Target AWS_ACCOUNT_ID NOT the Source
...
6. After updating the parameters, run:
    ...
    ./deploy.sh
    ```
If this is the first time you run the script, it will ask to create an S3 bucket to store CloudFormation artificats. Type ```y``` when prompted. Following that, the entire infrastructure required to replicate the Glue catalog from the source account will be deployed
